drop function if exists public.record_estimation_api_usage(text);

create function public.record_estimation_api_usage(p_ip_hash text)
returns table (
  ip_count integer,
...
begin
  ...
  returning estimation_api_usage.request_count into current_ip_count;
  ...
  on conflict (scope, identifier_hash, window_started_at)
  do update set request_count = usage.request_count + 1, updated_at = now()
  returning estimation_api_usage.request_count into current_global_count;
