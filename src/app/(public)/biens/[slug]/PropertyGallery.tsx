...
export function PropertyGallery({ images, title, isExclusive = false, isSold = false }: { images: GalleryImage[]; title: string; isExclusive?: boolean; isSold?: boolean }) {
  ...
  return <>
    <div className={styles.gallery}>
      <button aria-label="Voir toutes les photos" className={styles.mainImage} onClick={() => setSelected(0)} type="button">
        ...
        {isSold ? <em className={styles.gallerySold}>Vendu par Les Jumelles</em> : isExclusive ? <em className={styles.galleryExclusive}>Exclusivité Les Jumelles</em> : null}
    ...
    <div aria-label={`Galerie de ${images.length} photos, faites défiler horizontalement`} className={styles.mobileGallery}>{images.map((image, index) => <figure key={image.id}><Image alt={image.alt_text || `${title} — photo ${index + 1}`} fill priority={index === 0} sizes="100vw" src={image.public_url}/>{index === 0 && isSold ? <em className={styles.gallerySold}>Vendu par Les Jumelles</em> : isExclusive && index === 0 ? <em className={styles.galleryExclusive}>Exclusivité Les Jumelles</em> : null}<span>{index + 1} / {images.length}</span></figure>)}</div>
