...
export async function LocalAgencyCityPreview({ citySlug }: { citySlug: string }) {
...
async function renderLocalAgencyCityPage(citySlug: string, seoPreview: boolean) {
  ...
  const previewProfile = inseeProfile && !inseeProfile.demographics && previewLocalInfo
    ...
    : inseeProfile ?? (previewLocalInfo ? {
        cityName: city.name,
        inseeCode: city.inseeCode,
        vintage: previewLocalInfo.vintage ?? 2022,
        sourceUrl: "https://www.insee.fr/fr/statistiques/5359146",
        totalHousing: previewLocalInfo.homes ?? 0,
        housingTypes: [],
        occupancy: [],
        tenure: [],
        rooms: [],
