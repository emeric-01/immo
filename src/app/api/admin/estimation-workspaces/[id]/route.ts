...
function parseUpdate(body: Partial<WorkspaceUpdate> | null): { ok: true; update: WorkspaceUpdate } | { error: string; ok: false } {
  ...
  const submittedBlocks = Array.isArray(body?.report_blocks) ? body.report_blocks : [];
  const blocks = submittedBlocks.filter((block): block is EstimationReportBlock => Boolean(block && allowed.has(block.id) && typeof block.enabled === "boolean"));
  if (!blocks.length || blocks.length !== submittedBlocks.length || new Set(blocks.map((block) => block.id)).size !== blocks.length) return { error: "La composition du rapport est invalide.", ok: false };
  const normalizedBlocks = normalizeReportBlocks(blocks);
