...
export default async function AudiencePage({ searchParams }: { searchParams: Promise<{ periode?: string }> }) {
  ...
  return <main className={admin.adminPage}>
    ...
    <section className={admin.content}><header className={admin.pageHeader}><div><p className={admin.eyebrow}>Mesure d’audience</p><h1>Trafic du site</h1><p>Une lecture claire des visites, des parcours et du trafic automatisé identifié.</p></div></header>
      ...
      <section className={styles.tables}><article className={styles.listCard}><span>Canaux</span><h2>Origine des entrées</h2><div className={styles.list}>{summary.channels.length ? summary.channels.map((item) => <div key={item.name}><span>{item.name}</span><strong>{item.value}</strong></div>) : <p className={styles.empty}>Les canaux apparaîtront avec les prochaines visites.</p>}</div></article><article className={styles.listCard}><span>Campagnes</span><h2>UTM et apporteurs</h2><div className={styles.list}>{summary.campaigns.length ? summary.campaigns.map((item) => <div key={item.name}><span>{item.name}</span><strong>{item.value}</strong></div>) : <p className={styles.empty}>Aucune campagne sur cette période.</p>}</div></article><article className={styles.listCard}><span>Leads</span><h2>Conversions enregistrées</h2><div className={styles.list}>{summary.conversionKinds.length ? summary.conversionKinds.map((item) => <div key={item.name}><span>{item.name}</span><strong>{item.value}</strong></div>) : <p className={styles.empty}>Aucune conversion sur cette période.</p>}</div></article></section>
