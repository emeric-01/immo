...
export default async function PropertiesAdminPage() {
  ...
  const [canCreate, canManageAll, canUpdateOwn] = await Promise.all([
    hasAdminPermission(session, "properties:create"),
    hasAdminPermission(session, "properties:write"),
    hasAdminPermission(session, "properties:update_own"),
  ]);
  ...
  return <main className={admin.adminPage}>
    ...
    <section className={`${admin.content} ${styles.catalogContent}`}>
      <header className={admin.pageHeader}><div><p className={admin.eyebrow}>Catalogue immobilier</p><h1>Vos biens</h1><p>Gérez vos annonces, leur publication et leur visibilité depuis un seul espace.</p></div>{canCreate ? <Link className={styles.addButton} href="/admin/biens/nouveau"><Plus size={18}/> Ajouter un bien</Link> : null}</header>
      ...
      {!error && publicProperties.length && canManageAll ? <PropertyOrderManager properties={publicProperties}/> : null}
      ...
      {error ? <p className={styles.catalogError}>{error}</p> : properties.length === 0 ? <section className={styles.emptyCatalog}><Home/><h2>Aucun bien pour le moment</h2><p>Ajoutez votre première annonce pour commencer.</p>{canCreate ? <Link className={styles.addButton} href="/admin/biens/nouveau"><Plus size={18}/> Ajouter un bien</Link> : null}</section> : <div className={styles.list}>{properties.map(property => { const marketScore = marketScores.get(property.id); const canEdit = canManageAll || (canUpdateOwn && property.created_by_admin_id === session.id); return <article key={property.id}>
        ...
        <div className={styles.rowActions}>{canEdit ? <Link href={`/admin/biens/${property.id}`}><Pencil size={17}/> Modifier</Link> : null}{canManageAll ? <DeletePropertyButton propertyId={property.id} propertyTitle={property.title}/> : null}{canEdit ? <Link href={`/admin/biens/${property.id}/statistiques`}><BarChart3 size={17}/> Statistiques</Link> : null}<Link href={`/biens/${property.slug}${["published", "sold"].includes(property.status) ? "" : "?apercu=1"}`} target="_blank"><ExternalLink size={17}/> {["published", "sold"].includes(property.status) ? "Voir" : "Aperçu"}</Link>{canEdit ? <a href={`/admin/api/properties/${property.id}/pdf`} target="_blank"><Image alt="" aria-hidden="true" height={18} src="/icons/pdf.svg" width={18}/> PDF</a> : null}</div>
      </article>})}</div>}
    </section>
  </main>;
}
