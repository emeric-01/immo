...
import { Archive, BarChart3, Building2, ExternalLink, Home, Images, Info, Pencil, Plus } from "lucide-react";
...
export default async function PropertiesAdminPage() {
  ...
  return <main className={admin.adminPage}>
    ...
    <section className={`${admin.content} ${styles.catalogContent}`}>
      ...
      <div className={styles.catalogStats}><article><Building2/><span>Total</span><strong>{properties.length}</strong></article><article><span className={styles.liveDot}/><span>Publiés</span><strong>{published}</strong></article><article><Archive/><span>Brouillons</span><strong>{drafts}</strong></article><article><span className={styles.scoreDot}/><span>Prix cohérents</span><strong>{coherentProperties}/{scoredProperties.length}</strong></article></div>
      ...
      {error ? <p className={styles.catalogError}>{error}</p> : properties.length === 0 ? <section className={styles.emptyCatalog}><Home/><h2>Aucun bien pour le moment</h2><p>Ajoutez votre première annonce pour commencer.</p><Link className={styles.addButton} href="/admin/biens/nouveau"><Plus size={18}/> Ajouter un bien</Link></section> : <div className={styles.list}>{properties.map(property => { const marketScore = marketScores.get(property.id); return <article key={property.id}>
        ...
        <div className={styles.propertyInfo}><div><span className={styles.status} data-status={property.status}>{property.status === "published" ? "Publié" : property.status === "draft" ? "Brouillon" : "Archivé"}</span><span className={styles.propertyType}>{property.property_type === "house" ? "Maison" : property.property_type === "land" ? "Terrain" : "Appartement"}</span></div><h2>{property.title}</h2><p>{property.city_name}{property.postal_code ? ` (${property.postal_code})` : ""}</p><div className={styles.propertyMetrics}><strong>{new Intl.NumberFormat("fr-FR").format(property.price)} €</strong>{property.surface_m2 ? <span>{property.surface_m2} m²</span> : null}{property.rooms ? <span>{property.rooms} pièces</span> : null}<span><Images size={14}/> {property.images.length} photo{property.images.length > 1 ? "s" : ""}</span></div>{marketScore ? <div className={styles.marketScore} data-position={marketScore.marketPosition}><strong className={styles.marketPositionLabel}><i/>{marketScore.label}</strong><div><small>{new Intl.NumberFormat("fr-FR").format(marketScore.listingPricePerM2)} €/m² affiché · fourchette {new Intl.NumberFormat("fr-FR").format(marketScore.rangeLowPerM2)}–{new Intl.NumberFormat("fr-FR").format(marketScore.rangeHighPerM2)} €/m²</small></div><em>Prestations {marketScore.qualityAdjustmentPercent > 0 ? "+" : ""}{marketScore.qualityAdjustmentPercent}%</em><details className={styles.scoreInfo}><summary aria-label={`Comprendre l’analyse de prix de ${property.title}`} title="Comprendre l’analyse"><Info aria-hidden="true" size={15}/></summary><div className={styles.scorePopover}><strong>Comment cette position est-elle calculée ?</strong><p>Le prix affiché est comparé à une fourchette ajustée pour la typologie et les prestations du bien.</p><dl><div><dt>Position</dt><dd>{marketScore.label}</dd></div><div><dt>Écart à la référence</dt><dd>{marketScore.gapPercent > 0 ? "+" : ""}{marketScore.gapPercent}%</dd></div><div><dt>Typologie</dt><dd>{marketScore.propertyTypeLabel}</dd></div><div><dt>Référence</dt><dd>{marketScore.source}</dd></div><div><dt>Comparables retenus</dt><dd>{marketScore.comparableCount} vente{marketScore.comparableCount > 1 ? "s" : ""}</dd></div><div><dt>Prix de référence ajusté</dt><dd>{new Intl.NumberFormat("fr-FR").format(marketScore.referencePricePerM2)} €/m²</dd></div><div><dt>Pondération prestations</dt><dd>{marketScore.qualityAdjustmentPercent > 0 ? "+" : ""}{marketScore.qualityAdjustmentPercent}%</dd></div></dl><p className={styles.scoreMethod}>Comparables du même type, surface comprise entre 70 et 135 % et écart maximal d’une pièce. L’annonce est ensuite classée sous, dans ou au-dessus de la fourchette justifiée.</p>{marketScore.qualityFactors.length ? <ul>{marketScore.qualityFactors.map((factor) => <li key={factor}>{factor}</li>)}</ul> : <p>Aucune prestation valorisante ou dévalorisante renseignée.</p>}</div></details></div> : <div className={styles.marketScoreEmpty}>Analyse indisponible — données ville ou surface manquantes</div>}</div>
        <div className={styles.rowActions}><Link href={`/admin/biens/${property.id}`}><Pencil size={17}/> Modifier</Link><DeletePropertyButton propertyId={property.id} propertyTitle={property.title}/><Link href={`/admin/biens/${property.id}/statistiques`}><BarChart3 size={17}/> Statistiques</Link><Link href={`/biens/${property.slug}${property.status === "published" ? "" : "?apercu=1"}`} target="_blank"><ExternalLink size={17}/> {property.status === "published" ? "Voir" : "Aperçu"}</Link><a href={`/admin/api/properties/${property.id}/pdf`} target="_blank"><Image alt="" aria-hidden="true" height={18} src="/icons/pdf.svg" width={18}/> PDF</a></div>
      </article>})}</div>}
    </section>
  </main>;
}
