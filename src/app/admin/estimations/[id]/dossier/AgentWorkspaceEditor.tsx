...
function PriceHistoryChart({ history, propertyType }: { history: CityPriceHistoryPoint[]; propertyType: "apartment" | "house" }) {
  ...
  const left = 72; const right = 700; const top = 26; const bottom = 198;
  const path = chart.points.map((point, index) => `${index ? "L" : "M"} ${left + point.xRatio * (right - left)} ${top + point.yRatio * (bottom - top)}`).join(" ");
  return <><div className={styles.liveReportHeading}><div><p className={styles.liveReportKicker}>Évolution locale</p><h2>Prix au m² depuis {firstPeriod ? shortPeriod(firstPeriod) : "la première donnée disponible"}</h2></div>{chart.points.length > 1 ? <strong className={chart.delta >= 0 ? styles.trendPositive : styles.trendNegative}>{chart.delta >= 0 ? "+" : ""}{chart.delta.toLocaleString("fr-FR", { maximumFractionDigits: 1 })} % depuis {shortPeriod(firstPeriod)}</strong> : null}</div>{chart.points.length > 1 ? <div className={styles.livePriceChart}><div className={styles.chartTopValue}>{formatNumber(chart.points.at(-1)?.value ?? 0)} €/m²</div><svg aria-label="Évolution du prix moyen au mètre carré, avec échelles de prix et de dates" role="img" viewBox="0 0 720 240"><defs><linearGradient id="price-fill" x1="0" x2="0" y1="0" y2="1"><stop offset="0" stopColor="#bd7446" stopOpacity=".28"/><stop offset="1" stopColor="#bd7446" stopOpacity="0"/></linearGradient></defs><text className={styles.priceChartUnit} x="10" y="16">€/m²</text>{chart.yTicks.map((tick) => { const y = top + (chart.yMax - tick) / (chart.yMax - chart.yMin) * (bottom - top); return <g key={tick}><line x1={left} x2={right} y1={y} y2={y} /><text className={styles.priceChartTick} textAnchor="end" x={left - 10} y={y + 4}>{formatNumber(tick)}</text></g>; })}{chart.xTicks.map((tick, index) => { const x = left + tick.xRatio * (right - left); return <g key={tick.label}><line x1={x} x2={x} y1={top} y2={bottom} /><text className={styles.priceChartTick} textAnchor={index === 0 ? "start" : index === chart.xTicks.length - 1 ? "end" : "middle"} x={x} y="226">{shortPeriod(tick.label)}</text></g>; })}<path className={styles.priceChartArea} d={`${path} L ${right} ${bottom} L ${left} ${bottom} Z`} /><path className={styles.priceChartLine} d={path} /></svg><div className={styles.chartAxis}><span>{historyDurationLabel(firstPeriod, lastPeriod)} · prix moyen observé · {propertyType === "house" ? "maisons" : "appartements"}</span></div></div> : <p className={styles.liveBody}>L’historique pluriannuel n’est pas disponible dans ce dossier. Les 12 derniers mois restent présentés dans les repères marché.</p>}</>;
}
