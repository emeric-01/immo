...
import { ArrowLeft, BarChart3, CalendarDays, Euro, Gauge, Home, Mail, MapPin, Ruler, ShieldCheck, UserRound, UsersRound } from "lucide-react";
...
export default async function AdminEstimationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <Frame>
    ...
    <section className={styles.detailGrid}>
      ...
      <InfoPanel title="Attribution commerciale"><Metric icon={UsersRound} label="Agent commercial" value={commercialAgent?.full_name ?? "Aucun agent attribué"} /><Metric icon={Mail} label="E-mail de l’agent" value={commercialAgent?.email ?? "Non renseigné"} /><Metric icon={ShieldCheck} label="Origine" value={formatAdminAttribution(estimation.attribution_snapshot)} /><Metric icon={Gauge} label="Campagne" value={formatAdminAttributionCampaign(estimation.attribution_snapshot)} /></InfoPanel>
    </section>
  </Frame>;
}
